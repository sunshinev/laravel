...
class Container implements ArrayAccess, ContainerContract
{
     ...
     * 将绑定关系注册到容器里面，但是实际上就是将接口的实现，绑定到实现的类里面
    ...
    public function bind($abstract, $concrete = null, $shared = false)
    {
        ...
        // 删掉过期的实例？意思是，一旦进行了重新绑定，那么就要将之前的进行删除，进行重新绑定注册
        // 也是为了释放内存
        ...
        // 这个地方应该是判断，如果这个已经被绑定了，那么直接触发用来监听的回调函数
    ...
    protected function getClosure($abstract, $concrete)
    {
        ...
        //return $c->make('App\Console\Kernel',$params);
    }
    ...
    {
        // 通过绑定的实现来make接口或者抽象类，得到实例
        $instance = $this->make($abstract);
        // 不同的abstract有对象的rebound listener callbacks
    ...
    public function make($abstract, array $parameters = [])
    {
        ...
        // 这里就是得到$this->bindings 的concrete所对应的实现，就是获取一个类的实例
        $concrete = $this->getConcrete($abstract);
        
        // We're ready to instantiate an instance of the concrete type registered for
        // the binding. This will instantiate the types, as well as resolve any of
        // its "nested" dependencies recursively until all have gotten resolved.
        // 搞不懂这里为什么要将$abstract作为args[2]进行传递
        // 如果下面的递归仅仅为了满足buildable里面的===条件的话，何必传递$abstract，直接传递concrete就好了
        // 如果concrete是closure，或者 abstract === concrete，？但是什么情况下，abstract会=== concrete呢？
        if ($this->isBuildable($concrete, $abstract)) {
            // build利用反射机制
            $object = $this->build($concrete, $parameters);
        } else {
            // 递归，如果第一次buildable不成立，那么递归后一定成立
